...
<div class="container">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 p-3 bg-primary text-white rounded shadow-sm">
        <h1 class="h4 mb-2 mb-sm-0">
            <i class="bi bi-pencil-square me-2"></i> Edit Notification
        </h1>
            ...
            <form method="POST" action="{{ route('admin.notifications.update', $notification) }}" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="title" name="title" 
                           class="form-control" value="{{ old('title', $notification->title) }}" 
                ...
                <div class="mb-3">
                    ...
                    @if($notification->pdf_path)
                        <div class="p-2 bg-light border rounded mb-2">
                            ...
                            Current PDF: 
                            <a href="{{ asset('storage/' . $notification->pdf_path) }}" target="_blank" class="fw-semibold">
                                View PDF
                            </a>
                        </div>
                    @endif
                    <input type="file" id="pdf" name="pdf" accept="application/pdf" class="form-control">
                    <div class="form-text">Uploading a new file will replace the existing PDF.</div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select id="user_type" name="user_type" class="form-select" required>
                        <option value="public" {{ old('user_type', $notification->user_type) == 'public' ? 'selected' : '' }}>Public</option>
                        <option value="user" {{ old('user_type', $notification->user_type) == 'user' ? 'selected' : '' }}>Authenticated Users</option>
                        <option value="admin" {{ old('user_type', $notification->user_type) == 'admin' ? 'selected' : '' }}>Admins Only</option>
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select id="event_id" name="event_id" class="form-select">
                        ...
                        @foreach($events as $event)
                            <option value="{{ $event->id }}" {{ old('event_id', $notification->event_id) == $event->id ? 'selected' : '' }}>
